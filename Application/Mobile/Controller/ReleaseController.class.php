...
class ReleaseController extends GeneralController{
	...
	public function todo(){ 
		...
		try {
			$this->added($now_time);//未体现积分增值
 			$res = $this->getRelease($now_time);//待释放积分

			if(!empty($res)){
				$trade_fee_array = $res['trade_fee_array'];
				$uid_array = $res['uid_array'];
				$this->saveAll('__RELEASE__', $res['data'], $res['condition'], 1);//更新释放数据
				$this->addJF($trade_fee_array, $uid_array);//增加积分
			}
		} catch (Exception $e) {
			$M->rollback();
			exit();
	...
	private function added($now_time){
		...
		$where['add_time'] = array('elt',$now_time-60*$zzys); //change
		$uid_array = M('Release')->where($where)->field('uid')->group('uid')->select();
		$account = M('MemberAccount')->where(array('uid'=>array('in',array_column($uid_array, 'uid'))))->select();
		if(!empty($account)){
			...
			$this->saveAll('__MEMBER_ACCOUNT__', $data, $condition, 1);
	...
	private function addJF($trade_fee_array,$uid_array){
		...
		$this->saveAll('__MEMBER_ACCOUNT__', $data, $condition, 1);//更新释放数据
	...
	private function addLog($trade_fee_array,$uid_array,$desc = '释放增值积分'){
		...
		while (!empty($uid_array[$i])){
			$data[] = array(
					...
					'trade_code'=>'JF',
					'trade_type'=>4,
					'final_fee'=>$trade_fee_array[$i]*$proportion,
					'time_start'=>$now_time,
					'desc'=>$desc,
					'reward_code'=>'ZZJF',
			);
