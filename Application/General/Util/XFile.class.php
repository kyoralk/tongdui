 ...
 * 
 * 例子：
 * XFile::createDir('a/1/2/3');                    测试建立文件夹 建一个a/1/2/3文件夹
 * XFile::createFile('b/1/2/3');                    测试建立文件        在b/1/2/文件夹下面建一个3文件
 * XFile::createFile('b/1/2/3.exe');             测试建立文件        在b/1/2/文件夹下面建一个3.exe文件
 * XFile::copyDir('b','d/e');                    测试复制文件夹 建立一个d/e文件夹，把b文件夹下的内容复制进去
 * XFile::copyFile('b/1/2/3.exe','b/b/3.exe'); 测试复制文件        建立一个b/b文件夹，并把b/1/2文件夹中的3.exe文件复制进去
 * XFile::moveDir('a/','b/c');                    测试移动文件夹 建立一个b/c文件夹,并把a文件夹下的内容移动进去，并删除a文件夹
 * XFile::moveFile('b/1/2/3.exe','b/d/3.exe'); 测试移动文件        建立一个b/d文件夹，并把b/1/2中的3.exe移动进去                   
 * XFile::unlinkFile('b/d/3.exe');             测试删除文件        删除b/d/3.exe文件
 * XFile::unlinkDir('d');                      测试删除文件夹 删除d文件夹
 */
class XFile {
